...
- Audit every source, calculation and proxy for suitability, reproducibility and opportunities for more objective measures; record any resulting methodology changes and evidence limitations.
  ...
  - Finish the documented Residential environment replacement: review the selected country-calibrated noise/Energy Performance Certificate (EPC) method through the remaining weight-sensitivity/outlier/correlation/country-domain checks, and only then replace Local condition. See `docs/residential-environment-plan.md`.
