...
This compares National Rail Journey Planner access from one reviewed National Rail station per broad candidate location to fixed London and Birmingham rail gateways. It is a dated, station-to-station planner observation—not a property, neighbourhood, fare, accessibility or guaranteed journey measure. Planner-suggested public-transport connectors are in scope. Timetables, disruption and planner routing can change results. It is separate from the DfT local public-transport connectivity metric; neither measure may be used to infer the other.
...
Birmingham New Street to Birmingham New Street is a same-station self-route. Record it as 0 minutes and 0 changes, with an explicit reason and without claiming a planner capture.
...
Select the shortest elapsed itinerary departing within the window from the retained National Rail Journey Planner response. Do not add a mode-based eligibility filter: any public-transport route returned by that interface is in scope, including connectors such as Underground, bus, ferry or rail-replacement services. `changes` is the number of returned itinerary legs minus one, checked against the retained response rather than inferred from station names. If no returned itinerary is available, leave duration and changes blank and state why.

Where the retained responses cover it, count distinct returned planner departures in a recorded two-hour local window as `usable_departures_in_window`. It is snapshot context, not general frequency; leave it blank when it cannot be counted reliably.
